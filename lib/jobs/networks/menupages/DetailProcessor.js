function processPageData($, pageData, headers, processPageDataCallback) {
    ...
    try {
        ...
        container.find('#menu-tab .url').each(function(idx,elem){
            data.url=$(elem).text().replace('/menu','');
        })

        container.find('.title1respage').each(function (idx, elem) {
                    data.name = $(elem).text().trim();
                })
        container.find('.phonenew strong').each(function (idx, elem) {
            data.phone = $(elem).text();
        })

        container.find('.price-key').each(function (idx, elem) {
            data.priceString = $(elem).text();
        })

        container.find('.cuisine').each(function (idx, elem) {
            var cuisineString = $(elem).text();
            data.cuisines = cuisineString.split(',');
        })

        container.find('td.food-rating img').each(function (idx, elem) {
            var imgSrc = elem.src;
            if (imgSrc.indexOf('star') > -1) {
                imgSrc = imgSrc.substring(imgSrc.indexOf('star') + 4, imgSrc.length);
                imgSrc = imgSrc.replace('.gif', '').replace('_', '.');
                try {
                    data.rating = parseFloat(imgSrc);
                } catch (ex) {
                }
            }
        })
        ...
        var featureMap = {
            ...
            'Business Dining':'businessdining',
