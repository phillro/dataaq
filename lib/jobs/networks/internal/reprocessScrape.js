...
var ScrapeReprocessor = function (input, opts) {
    ...
    var methods = {
        run:function (scrape) {
          ...
          //          http://nymag.com/listings/restaurant/epicerie-boulud/index.html
            //        /pages/nymagpages/nymag.com/listings/restaurant/epicerie-boulud/index.html

                    var s3url = scrape.data.url.replace('http://nymag.com/','/pages/nymagpages/nymag.com/').replace('index.html','');
                    //s3.getFile('/pages/menupages/www.menupages.com/restaurants/kaijou/', function (err, res) {
                    s3.getFile(s3url, function (err, res) {
                        if (err) {
                            cb(err);
                        } else {
                            var body = '';
